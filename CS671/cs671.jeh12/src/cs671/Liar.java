...
public class Liar<T> implements Guesser<Liar.Secret<T>> {
	...
	public int selectCandidates(int n){
		...
		allCandidates = new ArrayList<T>(allCandidates.subList(0, retVal));
	...
	public Liar(Set<? extends T> candidates, int lies, String name){
		...
		this.allCandidates = new ArrayList<T>(candidates);
	}
	...
	public String initialize() {
		...
		this.upperPile = new TreeMap<Integer, ArrayList<T>>();
		this.lowerPile = new TreeMap<Integer, ArrayList<T>>();
		...
		this.upperPile.put(0, new ArrayList<T>((allCandidates).subList(0,numOfCandidates/2)));
		this.lowerPile.put(0, new ArrayList<T>((allCandidates).subList(numOfCandidates/2,numOfCandidates)));
	...
	public Secret<T> getSecret() {
		...
		for(int i = 0; i <= maxLies && retVal == null; i++){
			...
			if(!upperPile.get(i).isEmpty())
				retVal = new Secret<T>(i, upperPile.get(i).get(0));
			else if(!lowerPile.get(i).isEmpty())
				retVal = new Secret<T>(i, lowerPile.get(i).get(0));
	...
	public void yes() {
		...
		for(int i = maxLies; i >= 0; i--){
			ArrayList<T> temp = new ArrayList<T>(lowerPile.get(i));
		...
		for(int i = 0; i <= maxLies; i++){
			ArrayList<T> currentColumnValues = new ArrayList<T>();
			int numOfValues;
			
			ArrayList<T> temp = new ArrayList<T>(upperPile.get(i));
			for(T candidate: temp)
				currentColumnValues.add(candidate);
			
			temp = new ArrayList<T>(lowerPile.get(i));
			for(T candidate: temp)
				currentColumnValues.add(candidate);
